...
$sheet->setCellValue('A1','Tanggal Daftar');
...
while($row = mysqli_fetch_array($query))
{
	...
	$sheet->setCellValue('H'.$i, $row['noreg']);
	$sheet->setCellValue('I'.$i, $row['agama']);
	$sheet->setCellValue('J'.$i, $row['kwn']);
	$sheet->setCellValue('K'.$i, $row['kebutuhan']);
	...
	$sheet->setCellValue('U'.$i, $row['tinggal']);
	$sheet->setCellValue('V'.$i, $row['moda']);
	$sheet->setCellValue('W'.$i, $row['kks']);
	$sheet->setCellValue('Q'.$i, $row['anak']);
	$sheet->setCellValue('Y'.$i, $row['pkps']);
...
if (!empty($tanggal)&&!empty($nama)&&!empty($jk)&&!empty($nisn)&&!empty($nik)&&!empty($tempat)&&!empty($ttl)&&!empty($noreg)&&!empty($agama)&&!empty($kwn)&&!empty($kebutuhan)&&!empty($alamat)&&!empty($rt)&&!empty($rw)&&!empty($desa)&&!empty($kecamatan)&&!empty($pos)&&!empty($lintang)&&!empty($bujur)&&!empty($tinggal)&&!empty($moda)&&!empty($anak)&&!empty($pkps)) {
	...
	if (mysqli_query($con, $sql)) {
		echo "<script> 
            alert('Data berhasil dimasukkan ke Database dan MS.Excel!');
            document.location.href = '3.redirect.html';
        </script>";
	}else{
